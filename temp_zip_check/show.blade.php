...
<body class="max-w-5xl mx-auto p-4 md:p-8 relative min-h-screen flex flex-col z-0  ">
        ...
        <div class="text-right">
            ...
            <div class="mb-2">
                @if($invoice->status_pembayaran === 'lunas')
                    <span class="inline-block px-3 py-1 bg-green-100 text-green-800 border border-green-300 rounded font-bold text-xs uppercase">LUNAS</span>
                @else
                    <span class="inline-block px-3 py-1 bg-red-100 text-red-800 border border-red-300 rounded font-bold text-xs uppercase">BELUM LUNAS</span>
                @endif
            </div>
            @endif
            <table class="w-full text-right text-sm">
                <tr>
                    <td class="pr-2 py-1">Tanggal:</td>
                    <td class="font-semibold">{{ \Carbon\Carbon::parse($invoice->tanggal_invoice)->translatedFormat('d F Y') }}</td>
                </tr>
                <tr>
                    <td class="pr-2 py-1">Jatuh Tempo:</td>
    ...
    <!-- Tabel Item -->
    <div class="mt-8">
        <table class="w-full table-border border-collapse text-sm text-center">
            <thead>
                <tr class="font-bold bg-gray-100">
                    <th class="py-2 px-2 w-10">No</th>
                    <th class="py-2 px-2 w-24">Kode Barang</th>
                    <th class="py-2 px-2">Nama Barang / Jasa</th>
                    <th class="py-2 px-2 w-24">Kategori</th>
                    <th class="py-2 px-2">Harga Satuan</th>
                    <th class="py-2 px-2 w-14">Jumlah</th>
                    <th class="py-2 px-2 w-16">Satuan</th>
                ...
                <tr>
                    ...
                    <td class="py-2 px-2 text-left">
                        ...
                        @if(isset($item->product->merk))
                            <br><span class="text-[10px] text-gray-500 uppercase tracking-wider">{{ $item->product->merk->nama_merk }}</span>
                        @endif
                    </td>
            ...
            <!-- Footer Tabel (Perhitungan) -->
            <tfoot class="font-bold">
                <tr>
                    <td colspan="7" class="py-2 px-4 text-right border-r">Sub Total</td>
                    ...
                    <td class="py-2 px-2 text-center align-middle bg-yellow-50" rowspan="3" style="font-size: 1.1em;">
                        Rp{{ number_format($invoice->sub_total - $total_modal, 2, ',', '.') }}
                    </td>
                    @endif
                </tr>
                <tr>
                    <td colspan="7" class="py-2 px-4 text-right border-r">Pajak (PPN)</td>
                    <td class="py-2 px-2 text-right">Rp{{ number_format($invoice->pajak_ppn, 2, ',', '.') }}</td>
                </tr>
                <tr>
                    <td colspan="7" class="py-2 px-4 text-right border-r">Total Tagihan</td>
                    <td class="py-2 px-2 text-right">Rp{{ number_format($invoice->total_tagihan, 2, ',', '.') }}</td>
                </tr>
                <!-- Terbilang -->
                <tr class="italic">
                    <td colspan="{{ $totalColspan }}" class="py-2 px-4 text-left">
                        <span class="font-bold mr-2">Terbilang :</span> 
    ...
    <div class="mt-8 text-sm">
        ...
        <table class="w-full max-w-sm">
            <tr>
                <td class="w-20">Bank</td>
                <td class="font-bold">: BSI</td>
            </tr>
            <tr>
                <td>No. Rek</td>
                <td class="font-bold text-lg">: 7343793687</td>
            </tr>
            <tr>
                <td>A/N</td>
                <td class="font-bold">: PT Utama Madani Raya</td>
            </tr>
        </table>
    </div>
        
    <!-- Tanda Tangan -->
    <div class="mt-8 flex justify-end page-break-inside-avoid">
    ...
    <div class="print-footer w-full mt-auto">
        <div class="header-bg p-3 text-center text-sm font-semibold tracking-wide" style="border-radius: 4px; background-color: #111827; color: white;">
