...
<div x-data="purchaseEditForm({{ $products->toJson() }}, {{ $purchase->purchaseItems->toJson() }})" @click.away="closeAllDropdowns()">
    ...
    <form action="{{ route('purchases.update', $purchase) }}" method="POST" @submit.prevent="submitForm($event)">
        @csrf
        @method('PUT')
        ...
        <div class="page-header">
            <div>
                ...
                <p>Ubah detail nota pembelian barang masuk dan sesuaikan stok otomatis</p>
            </div>
            <div class="header-icon">
                <svg style="width:1.75rem;height:1.75rem;color:white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
            </div>
        </div>
        ...
        <div class="info-grid">
            <div class="info-card">
                ...
                <input type="text" name="nomor_faktur" value="{{ old('nomor_faktur', $purchase->nomor_faktur) }}" placeholder="Contoh: INV-001/VI/2026" required>
            </div>
            <div class="info-card">
                <label>🏪 Nama Supplier / Toko *</label>
                <input type="text" name="nama_supplier" value="{{ old('nama_supplier', $purchase->nama_supplier) }}" placeholder="Contoh: Toko Jaya Mas" required>
            </div>
            <div class="info-card">
            ...
            <div class="info-card">
                ...
                <input type="text" name="keterangan" value="{{ old('keterangan', $purchase->keterangan) }}" placeholder="Catatan tambahan...">
            </div>
        </div>
        ...
        <div class="action-bar">
            <div class="action-info">
                ...
                Stok barang master akan otomatis disesuaikan berdasarkan perubahan kuantitas
            </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('purchaseEditForm', (productsData, itemsData) => ({
        ...
        init() {
            this.items = itemsData.map(item => ({
                ...
                subtotal: (parseInt(item.harga_beli) || 0) * (parseInt(item.jumlah) || 1),
                searchQuery: '',
                searchResults: [],
                showDropdown: false,
                highlightIndex: -1,
                selectedName: item.product ? item.product.nama_barang : '',
                selectedSku: item.product ? item.product.sku : '',
                ddTop: 0,
                ddLeft: 0,
                ddWidth: 320,
            }));
