...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-sm text-gray-500">Ubah data pengeluaran operasional.</p>
        </div>
    </div>
    ...
    <form action="{{ route('expenses.update', $expense) }}" method="POST" class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="kategori" required class="w-full border px-4 py-3 bg-gray-50 rounded-xl border-gray-300 focus:bg-white focus:border-tema-marun focus:ring-2 focus:ring-tema-marun/20">
                    ...
                    <option value="Pembelian Barang" {{ old('kategori', $expense->kategori) == 'Pembelian Barang' ? 'selected' : '' }}>Pembelian Barang (Faktur Masuk)</option>
        ...
        <div>
            ...
            <input type="number" name="nominal" value="{{ old('nominal', (int)$expense->nominal) }}" required min="0" step="1"
                class="w-full border px-4 py-3 bg-gray-50 rounded-xl border-gray-300 focus:bg-white focus:border-tema-marun focus:ring-2 focus:ring-tema-marun/20">
        </div>
        ...
        <div>
            ...
            <textarea name="keterangan" rows="3" required
                class="w-full border px-4 py-3 bg-gray-50 rounded-xl border-gray-300 focus:bg-white focus:border-tema-marun focus:ring-2 focus:ring-tema-marun/20">{{ old('keterangan', $expense->keterangan) }}</textarea>
        </div>
