...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Edit Slip Gaji</h1>
            <p class="text-sm text-gray-500">Perbarui rincian gaji karyawan.</p>
        </div>
    </div>
    ...
    <form action="{{ route('slip-gaji.update', $slipGaji->id) }}" method="POST" id="salaryForm" class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" value="{{ $slipGaji->nomor_slip }}" disabled
                ...
                <div>
                    ...
                    <input type="text" name="jabatan" value="{{ old('jabatan', $slipGaji->jabatan) }}" placeholder="Contoh: Staf Administrasi"
                           class="w-full border px-4 py-3 bg-gray-50 rounded-xl border-gray-300 focus:bg-white focus:border-tema-marun focus:ring-2 focus:ring-tema-marun/20 outline-none transition-all">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Periode Slip Gaji</label>
                    <input type="text" name="periode" value="{{ old('periode', $slipGaji->periode) }}" required placeholder="Contoh: Juni 2026"
                           class="w-full border px-4 py-3 bg-gray-50 rounded-xl border-gray-300 focus:bg-white focus:border-tema-marun focus:ring-2 focus:ring-tema-marun/20 outline-none transition-all">
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="number" name="gaji_pokok" id="gaji_pokok" value="{{ old('gaji_pokok', (int) $slipGaji->gaji_pokok) }}" required min="0"
                ...
                <div>
                    ...
                    <input type="number" name="lembur" id="lembur" value="{{ old('lembur', (int) $slipGaji->lembur) }}" min="0"
                           class="calc-trigger w-full border px-4 py-2.5 bg-gray-50 rounded-xl border-gray-300 font-semibold focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 outline-none transition-all">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1">Tunjangan / Bonus (Rp)</label>
                    <input type="number" name="tunjangan_bonus" id="tunjangan_bonus" value="{{ old('tunjangan_bonus', (int) $slipGaji->tunjangan_bonus) }}" min="0"
                           class="calc-trigger w-full border px-4 py-2.5 bg-gray-50 rounded-xl border-gray-300 font-semibold focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 outline-none transition-all">
                </div>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="number" name="bpjs_kesehatan" id="bpjs_kesehatan" value="{{ old('bpjs_kesehatan', (int) $slipGaji->bpjs_kesehatan) }}" min="0"
                ...
                <div>
                    ...
                    <input type="number" name="bpjs_ketenagakerjaan" id="bpjs_ketenagakerjaan" value="{{ old('bpjs_ketenagakerjaan', (int) $slipGaji->bpjs_ketenagakerjaan) }}" min="0"
                           class="calc-trigger w-full border px-4 py-2.5 bg-gray-50 rounded-xl border-gray-300 font-semibold focus:bg-white focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 outline-none transition-all">
                </div>
            </div>
        </div>
        ...
        <div class="rounded-2xl p-5 flex flex-col sm:flex-row justify-between items-center gap-4 text-white shadow-md"
            ...
            <div>
                <p style="font-size:0.65rem;font-weight:800;color:#fecaca;text-transform:uppercase;letter-spacing:0.08em;">Total Gaji Bersih Diterima</p>
        ...
        <div>
            ...
            <textarea name="catatan" rows="3" placeholder="Tuliskan keterangan tambahan jika diperlukan..."
                      class="w-full border px-4 py-3 bg-gray-50 rounded-xl border-gray-300 focus:bg-white focus:border-tema-marun focus:ring-2 focus:ring-tema-marun/20 outline-none transition-all">{{ old('catatan', $slipGaji->catatan) }}</textarea>
        </div>
