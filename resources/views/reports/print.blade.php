...
<body class="max-w-5xl mx-auto p-4 md:p-8 relative min-h-screen flex flex-col z-0">
    ...
    <div class="text-center mb-8">
        <h2 class="text-xl font-bold uppercase underline">Laporan Rekapitulasi Invoice</h2>
    ...
    <div class="grid grid-cols-3 gap-4 mb-8">
        <div class="border-2 border-gray-800 rounded-lg p-4 text-center">
            ...
            <h3 class="text-lg font-black rp-num">Rp {{ number_format($totalPendapatan, 0, ',', '.') }}</h3>
        </div>
        <div class="border-2 border-gray-800 rounded-lg p-4 text-center">
            <p class="text-[10px] font-bold uppercase tracking-wider text-gray-600 mb-1">Total HPP (Modal)</p>
            <h3 class="text-lg font-black rp-num">Rp {{ number_format($totalModal, 0, ',', '.') }}</h3>
        </div>
        <div class="border-2 border-gray-800 rounded-lg p-4 text-center bg-gray-50">
            ...
            <h3 class="text-xl font-black rp-num">Rp {{ number_format($labaKotor, 0, ',', '.') }}</h3>
        </div>
    </div>

    <!-- Tabel Rincian -->
    <h3 class="font-bold text-sm mb-2 uppercase">Rincian Transaksi (Invoice)</h3>
    <div class="watermark-container">
        ...
        <thead>
            <tr>
                ...
                <th class="py-2 px-2 text-right w-28">Pendapatan</th>
                <th class="py-2 px-2 text-right w-28">Modal (HPP)</th>
        ...
        <tbody>
            @forelse($invoices as $idx => $inv)
                ...
                <tr>
                    ...
                    <td class="py-1.5 px-2 text-right rp-num">Rp {{ number_format($inv->sub_total, 0, ',', '.') }}</td>
                    <td class="py-1.5 px-2 text-right rp-num">Rp {{ number_format($modalInv, 0, ',', '.') }}</td>
                    <td class="py-1.5 px-2 text-right font-bold rp-num">Rp {{ number_format($labaInv, 0, ',', '.') }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="7" class="py-4 text-center italic text-gray-500">Tidak ada transaksi pada periode ini.</td>
                </tr>
            @endforelse
        </tbody>
        <tfoot class="font-bold bg-gray-100">
            <tr>
                <td colspan="4" class="py-2 px-2 text-right uppercase">Total Rekapitulasi :</td>
                <td class="py-2 px-2 text-right rp-num">Rp {{ number_format($totalPendapatan, 0, ',', '.') }}</td>
                <td class="py-2 px-2 text-right rp-num">Rp {{ number_format($totalModal, 0, ',', '.') }}</td>
                <td class="py-2 px-2 text-right rp-num">Rp {{ number_format($labaKotor, 0, ',', '.') }}</td>
            </tr>
        </tfoot>
    </table>
    </div>
