...
<head>
    ...
    <title>Laporan Pembelian - PT Utama Madani Raya</title>
...
<body class="max-w-5xl mx-auto p-4 md:p-8 relative min-h-screen flex flex-col z-0  ">
    ...
    <div class="text-center mb-8">
        <h2 class="text-xl font-bold uppercase underline">LAPORAN PEMBELIAN PER PERIODE</h2>
    ...
    <div class="watermark-container">
        ...
        <thead>
            <tr>
                ...
                <th class="py-2 px-2">No. Faktur</th>
                <th class="py-2 px-2">Nama Supplier</th>
                <th class="py-2 px-2 text-right w-32">Total Biaya (Rp)</th>
            </tr>
        </thead>
        <tbody>
            @forelse($purchases as $idx => $p)
                <tr>
                    ...
                    <td class="py-2 px-2 text-center">{{ \Carbon\Carbon::parse($p->tanggal_pembelian)->format('d/m/Y') }}</td>
                    <td class="py-2 px-2 font-bold">{{ $p->nomor_faktur }}</td>
                    <td class="py-2 px-2">{{ $p->nama_supplier }}</td>
                    <td class="py-2 px-2 text-right rp-num">Rp {{ number_format($p->total_biaya, 0, ',', '.') }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="5" class="py-4 text-center italic text-gray-500">Tidak ada data pembelian pada periode ini.</td>
                </tr>
            @endforelse
        </tbody>
        <tfoot class="font-bold bg-gray-100">
            <tr>
                <td colspan="4" class="py-2 px-2 text-right uppercase">TOTAL NILAI PEMBELIAN :</td>
                <td class="py-2 px-2 text-right text-sm font-black rp-num">Rp {{ number_format($totalPembelian, 0, ',', '.') }}</td>
            </tr>
        </tfoot>
    </table>
    </div>
