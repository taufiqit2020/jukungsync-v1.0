...
<head>
    ...
    <title>Laporan Bukti Claim Invoice - PT Utama Madani Raya</title>
...
<body class="max-w-5xl mx-auto p-4 md:p-8 relative min-h-screen flex flex-col z-0  ">
    ...
    <div class="text-center mb-8">
        <h2 class="text-xl font-bold uppercase underline">Laporan Bukti Claim Invoice</h2>
    ...
    <div class="watermark-container">
        ...
        <thead>
            <tr>
                ...
                <th class="py-2 px-2 w-48">Nama Klien</th>
                <th class="py-2 px-2 text-center w-[1000px]">Upload Gambar Bukti</th>
        ...
        <tbody>
            @forelse($invoices as $idx => $inv)
                <tr>
                    ...
                    <td class="py-1.5 px-2 text-center align-top">
                        @if(Str::endsWith($inv->bukti_file, '.pdf'))
                            <span class="font-bold text-[10px] text-gray-600 bg-gray-100 px-2 py-1 rounded border border-gray-300 inline-block">[Terlampir PDF]</span>
                        @else
                            <img src="{{ asset('storage/' . str_replace('public/', '', $inv->bukti_file)) }}" alt="Bukti" class="w-[1000px] max-w-full h-auto object-contain mx-auto border border-gray-200 rounded p-1">
                        @endif
                    </td>
                    <td class="py-1.5 px-2 align-top text-[10px]">
                        {{ $inv->bukti_keterangan ?: '-' }}
                    </td>
                </tr>
            @empty
                <tr>
                    <td colspan="6" class="py-4 text-center italic text-gray-500">Tidak ada bukti claim invoice pada periode ini.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
