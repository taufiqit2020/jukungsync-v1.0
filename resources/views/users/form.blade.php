            ...
            <div class="p-6 md:p-8 space-y-6">
                ...
                <div>
                    ...
                    <select name="role" class="w-full border px-4 py-3 bg-gray-50 rounded-xl border-gray-300 shadow-sm focus:bg-white focus:border-tema-marun focus:ring focus:ring-tema-marun focus:ring-opacity-50 transition-colors" required>
                        ...
                        @php $roles = ['superadmin' => 'Super Admin (Staff IT)', 'direktur' => 'Direktur (CEO)', 'bendahara' => 'KEPALA ADMINISTRASI DAN KEUANGAN', 'staf_admin' => 'Staf Admin (Gudang & Penjualan)', 'customer' => 'Pelanggan (Customer)']; @endphp
                ...
                <!-- Tipe Pelanggan -->
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Tipe Pelanggan <span class="text-gray-400 font-normal">(Khusus Role Customer)</span></label>
                    <select name="tipe_pelanggan" class="w-full border px-4 py-3 bg-gray-50 rounded-xl border-gray-300 shadow-sm focus:bg-white focus:border-tema-marun focus:ring focus:ring-tema-marun focus:ring-opacity-50 transition-colors">
                        <option value="reguler" {{ old('tipe_pelanggan', $user->tipe_pelanggan ?? 'reguler') == 'reguler' ? 'selected' : '' }}>⬜ Reguler — Harga jual standar | Bayar: Tunai / Transfer</option>
                        <option value="member" {{ old('tipe_pelanggan', $user->tipe_pelanggan ?? '') == 'member' ? 'selected' : '' }}>🔵 Member — Harga grosir min 12 pcs + diskon 0.2% | Bayar: Tunai / Transfer</option>
                        <option value="premium" {{ old('tipe_pelanggan', $user->tipe_pelanggan ?? '') == 'premium' ? 'selected' : '' }}>🌟 Premium — Harga grosir selalu + diskon 0.3% | Bayar: Tunai / Transfer / Invoice 30 Hari</option>
                    </select>
                    <p class="text-xs text-gray-400 mt-1.5">Tier menentukan harga yang berlaku, diskon, dan metode pembayaran yang tersedia saat Customer checkout.</p>
                </div>
