...
$css_replace = <<<EOD
        ...
        /* Custom Table Borders untuk menghindari Kop Surat kena border */
        .my-table {
        ...
        .my-table th { background-color: #f3f4f6; }
...
$content = preg_replace('/@media print\s*\{.*?\/\*\s*For screen view\s*\*\/\s*@media screen\s*\{[^}]+\}.*?\.watermark\s*\{[^}]+\}/s', $css_replace, $content);
...
<body class="max-w-5xl mx-auto p-4 md:p-8 relative">
    ...
    <table class="w-full my-table text-sm text-center bg-transparent inner-table">
        <thead class="bg-transparent">
            ...
            <tr class="font-bold bg-gray-100 uppercase text-xs">
        ...
        <tbody class="bg-transparent">
            ...
            @foreach(\$invoice->invoiceItems as \$index => \$item)
                ...
                @endphp
            <tr>
                <td class="py-2 px-2 bordered">{{ \$index + 1 }}</td>
                <td class="py-2 px-2 font-mono text-xs bordered">{{ \$item->product->sku ?? '-' }}</td>
                <td class="py-2 px-2 text-left bordered">
                    <span class="font-bold">{{ \$item->product->nama_barang ?? 'Barang Dihapus' }}</span>
                </td>
                <td class="py-2 px-2 text-xs uppercase bordered">{{ \$item->product->merk->nama_merk ?? '-' }}</td>
                <td class="py-2 px-2 uppercase bordered">PCS</td>
                <td class="py-2 px-2 text-xs bordered">{{ \$item->product->category->nama_kategori ?? '-' }}</td>
                <td class="py-2 px-2 bordered">{{ \$item->jumlah }}</td>
                <td class="py-2 px-2 text-right bordered">Rp {{ number_format(\$item->harga_jual_snapshot, 0, ',', '.') }}</td>
                <td class="py-2 px-2 text-right bordered">Rp {{ number_format(\$item->total_harga, 0, ',', '.') }}</td>
                
                @if(\$isInternal)
                <td class="py-2 px-2 text-right bordered">Rp {{ number_format(\$item->harga_modal_snapshot, 0, ',', '.') }}</td>
                <td class="py-2 px-2 text-right bordered">Rp {{ number_format(\$modal_item, 0, ',', '.') }}</td>
                <td class="py-2 px-2 text-right bordered border-l">Rp {{ number_format(\$keuntungan_item, 0, ',', '.') }}</td>
                @endif
            </tr>
            @endforeach
            ...
            <tr>
                <td class="py-2 px-2 text-transparent bordered">-</td>
                <td class="py-2 px-2 text-transparent bordered">-</td>
                <td class="py-2 px-2 text-transparent bordered">-</td>
                <td class="py-2 px-2 text-transparent bordered">-</td>
                <td class="py-2 px-2 text-transparent bordered">-</td>
                ...
                <td class="py-2 px-2 text-transparent bordered">-</td>
                <td class="py-2 px-2 text-transparent bordered">-</td>
                <td class="py-2 px-2 text-transparent bordered border-l">-</td>
                @endif
            </tr>
            @endfor
            
            <!-- TOTALS -->
            <tr class="font-bold" style="border-top: 2px solid black;">
                <td colspan="8" class="py-2 px-4 text-right bordered border-r">Sub Total</td>
                <td class="py-2 px-2 text-right bordered">Rp {{ number_format(\$invoice->sub_total, 0, ',', '.') }}</td>
                
                @if(\$isInternal)
                <td colspan="2" class="py-2 px-2 bordered bg-yellow-50 text-center align-middle" rowspan="3" style="font-size: 1.1em;">Rp {{ number_format(\$total_modal, 0, ',', '.') }}</td>
                <td class="py-2 px-2 text-center bordered align-middle bg-yellow-50 border-l" rowspan="3" style="font-size: 1.1em;">
                    Rp {{ number_format(\$invoice->sub_total - \$total_modal, 0, ',', '.') }}
                </td>
                @endif
            </tr>
            <tr class="font-bold">
                <td colspan="8" class="py-2 px-4 text-right bordered border-r">Pajak (PPN)</td>
                <td class="py-2 px-2 text-right bordered">Rp {{ number_format(\$invoice->pajak_ppn, 0, ',', '.') }}</td>
            </tr>
            <tr class="font-bold">
                <td colspan="8" class="py-2 px-4 text-right bordered border-r">Total Tagihan</td>
                <td class="py-2 px-2 text-right bordered">Rp {{ number_format(\$invoice->total_tagihan, 0, ',', '.') }}</td>
            </tr>
            <!-- Terbilang -->
    ...
    <!-- Banner Footer -->
    <div class="print-footer header-bg p-2 text-center text-sm font-semibold tracking-wide" style="border-radius: 4px; background-color: #111827; color: white;">
