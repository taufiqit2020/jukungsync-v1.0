...
return new class extends Migration
{
    ...
    public function up(): void
    {
        if (DB::getDriverName() === 'sqlite') {
            ...
            DB::statement("UPDATE users SET role = 'staf_admin' WHERE role = 'superadmin' AND id != (SELECT min_id FROM (SELECT MIN(id) AS min_id FROM users WHERE role = 'superadmin') AS temp)");
            ...
            // Langkah 1: Ubah semua nilai 'staff_gudang' yang lama menjadi 'staf_admin'
            DB::statement("UPDATE users SET role = 'staf_admin' WHERE role = 'staff_gudang'");
            DB::statement("UPDATE users SET role = 'staf_admin' WHERE role = 'superadmin' AND id != (SELECT min_id FROM (SELECT MIN(id) AS min_id FROM users WHERE role = 'superadmin') AS temp)");

            // Langkah 2: Gunakan ALTER TABLE di MySQL untuk memperbarui ENUM
            DB::statement("ALTER TABLE users MODIFY COLUMN role ENUM('superadmin', 'direktur', 'bendahara', 'staf_admin', 'customer') NOT NULL DEFAULT 'staf_admin'");
        }
    }
    ...
    public function down(): void
    {
        if (DB::getDriverName() === 'sqlite') {
            ...
            DB::statement("ALTER TABLE users MODIFY COLUMN role ENUM('superadmin', 'staff_gudang') NOT NULL DEFAULT 'staff_gudang'");
        }
    }
};
